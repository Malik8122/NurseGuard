...
function StatCard({ icon: Icon, iconColor, iconBg, label, value, sub, subOk, delay = 0 }: any) {
  return (
    ...
    >
      ...
      <p className="text-4xl font-extrabold leading-none tracking-tight" style={{ color: 'var(--text)' }}>
        {value ?? '—'}
      </p>
    </motion.div>
  )
...
function CoverageBar({ label, actual, required, color, delay = 0 }: any) {
  ...
  return (
    <div className="mb-4 last:mb-0">
      ...
      <p className="text-[11px] mt-1 text-right" style={{ color: 'var(--muted)' }}>{pct}%</p>
    </div>
  )
}
  ...
  return (
    <div className="page">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-7">
        <StatCard delay={0.0} icon={Users}         iconColor="text-blue-600"   iconBg="bg-blue-50"
          label="Total Nurses"    value={data?.total_nurses ?? 0}
          sub={data?.total_nurses ? `${data.on_duty ?? 0} on duty` : 'No data'}
          subOk={!!data?.total_nurses} />
        <StatCard delay={0.06} icon={Calendar}     iconColor="text-emerald-600" iconBg="bg-emerald-50"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="font-bold text-[15px] leading-none" style={{ color: 'var(--text)' }}>
                Today's Coverage
              </h2>
        ...
        >
          ...
          <p className="text-[12px] mt-4 text-center" style={{ color: 'var(--muted)' }}>
            Total registered: <strong style={{ color: 'var(--text)' }}>{data?.total_nurses ?? 0}</strong>
          </p>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-[12px] mt-1.5" style={{ color: 'var(--muted)' }}>Requires attention</p>
            </div>
          ...
          {data?.alerts?.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-[14px] font-bold" style={{ color: 'var(--text)' }}>All clear</p>
              <p className="text-[12px] mt-1" style={{ color: 'var(--muted)' }}>No active alerts right now</p>
            </div>
          )}
        </motion.div>
      </div>
